...
[  "-batch" = "$1" ] &&
exec $JAVA $MEMORY $SFTP_KEY -jar "$OLV_HOME/lib/olv-exec.jar" $@
...
exec $JAVA $MEMORY $SFTP_KEY  -DsingleInstance.startPort= -jar "$OLV_HOME/lib/olv-exec.jar" $@ > "${TMPDIR}/olv-${LOGNAME}.log" 2>&1 &
